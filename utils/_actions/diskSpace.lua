...
local bytesToTB = 1024 * 1024 * 1024 * 1024

local module = {}

module.textStyle = {
    font = { name = "Menlo", size = 12 },
    color = { alpha = 1.0, white = 1 },
    paragraphStyle = { alignment = "center" },
}
module.capacityColor  = { list = "x11", name = "orangered" }
module.availableColor = { list = "x11", name = "deepskyblue" }
...
module.getStats = function()
    ...
    for i,v in fnutils.sortByKeys(host.volumeInformation()) do
        local totalCapacity = round(v.NSURLVolumeTotalCapacityKey / bytesToGB)
        local totalCapacityUnit = " GB"
        if totalCapacity > 1024 then
            totalCapacity = round(v.NSURLVolumeTotalCapacityKey / bytesToTB)
            totalCapacityUnit = " TB"
        end
        local availableCapacity = round(v.NSURLVolumeAvailableCapacityKey / bytesToGB)
        local availableCapacityUnit = " GB"
        if availableCapacity > 1024 then
            availableCapacity = round(v.NSURLVolumeAvailableCapacityKey / bytesToTB)
            availableCapacityUnit = " TB"
        end
